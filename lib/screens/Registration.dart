                              ...
                              if (nameController.text != "" &&
                                ...
                                if (UserData.PASSWORD == password) {
                                  ...
                                  await LoadXlsx("assets/file.xlsx", "Motor")
                                    ...
                                    int totalGroups =
                                        ((listoo.length + motorIndexedQuestions.length ) / 100).ceil();
                                    ...
                                    for (var p = 0; p < totalGroups; p++) {
                                      groupId = p + 1;
                                      List<Question> groupIndexedQuestions = List<Question>();
                                      if (groupId < totalGroups) {
                                        int indexedLength =(motorIndexedQuestions.length /totalGroups).ceil();
                                        groupIndexedQuestions = motorIndexedQuestions.sublist(initIndexForIndexed , (initIndexForIndexed+indexedLength ));
                                        questions = listoo.sublist(
                                            initIndex, (initIndex + 100 - indexedLength )  , );
                                        initIndexForIndexed += indexedLength ;
                                        initIndex += (100 - indexedLength );
                                      } else {
                                        groupIndexedQuestions = this.motorIndexedQuestions.sublist(initIndexForIndexed);
                                        questions = listoo.sublist(initIndex);
                                      }
                                      // initIndex += 100;
                                      this.groutCounter++;

                                      for(var a=0; a < groupIndexedQuestions.length ; a++){
                                  ...
                                  await LoadXlsx("assets/file.xlsx", "Others")
                                    ...
                                    int initIndexForIndexed =0;
                                    int totalGroups =
                                        ((listoo.length + otherIndexedQuestions.length) / 100).ceil();
                                    List<List<String>> questions = [];
                                    List<Question> QuestObjects = [];
                                    List<Group> GroupObjects = [];
                                    List<GradeResult> gradeResultObjects = [];
                                    for (var p = 0; p < totalGroups; p++) {
                                      ...
                                      if (groupId < totalGroups) {
                                        int indexedLength = (otherIndexedQuestions.length / totalGroups).ceil();
                                        groupIndexedQuestions = otherIndexedQuestions.sublist(initIndexForIndexed , (initIndexForIndexed+indexedLength ));
                                        questions = listoo.sublist(
                                            initIndex, (initIndex + 100 - indexedLength ));
                                        initIndex+= ( 100 - indexedLength);
                                        initIndexForIndexed += indexedLength;
                                      } else {
                                        groupIndexedQuestions = otherIndexedQuestions.sublist(initIndexForIndexed);
                                        questions = listoo.sublist(initIndex);
                                      }
                                      // initIndex += 100;
                                      this.groutCounter++;
